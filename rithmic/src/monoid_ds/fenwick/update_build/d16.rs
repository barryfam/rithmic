...
impl<T, O: MonoidOps<T, T>> NdFenwick<16, T, O>
where T: Default
{
    pub fn update(&mut self, index: [usize; 16], value: T)
    {
        assert!(self.inbounds(index), "index outside of shape: {:?} / {:?}", index, self.shape());

        let mut i15 = index[15];
        while i15 < self.shape()[15] {
            let mut i14 = index[14];
            while i14 < self.shape()[14] {
                let mut i13 = index[13];
                while i13 < self.shape()[13] {
                                                            ...
                                                            while i2 < self.shape()[2] {
                                                                ...
                                                                while i1 < self.shape()[1] {
                                                                    ...
                                                                    while i0 < self.shape()[0] {
                                                                        let index = [i0, i1, i2, i3, i4, i5, i6, i7, i8, i9, i10, i11, i12, i13, i14, i15];
